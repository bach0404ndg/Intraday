...
class StrategyConfig:
    ...
    summary_start: date = date(2026, 6, 1)
    summary_end: date = date(2026, 9, 1)
...
def add_gap_features(data):
    ...
    open_930 = (
        data[data["timestamp"].dt.time == time(9, 30)]
        .set_index("date")["open"]
    )
    close_4 = (
        data[data["timestamp"].dt.time == time(15, 55)]
        .set_index("date")["close"]
    )

    daily_change = (close_4 - open_930) / open_930
    ...
    data["current_gap"] = (data["close"] - data["prev_close"]) / data["prev_close"]
    data["gap_change"] = data.groupby("date")["current_gap"].diff()
    data["daily_change"] = data["date"].map(daily_change)

    return data


def price_slope(values):
    time_index = np.arange(len(values))
    return np.polyfit(time_index, values, 1)[0]


def rolling_slope_by_day(data, column, bars):
    return (
        data.groupby("date")[column]
        .transform(lambda x: x.shift(1).rolling(bars).apply(price_slope, raw=True))
    )
...
def add_buy_signals(data, config):
    ...
    data["slope"] = rolling_slope_by_day(
        data,
        column="close",
    ...
    data["buy_signal"] = (
        ...
        & (data["execution_time"].dt.time < config.latest_buy_time)
    )

    data["first_buy_signal"] = (
        data["buy_signal"] & (data.groupby("date")["buy_signal"].cumsum() == 1)
    )
...
def add_sell_signals(data, config):
    ...
    data["slope_sell"] = rolling_slope_by_day(
        data,
        column="close",
    ...
    data["force_exit"] = (
        ...
        & (data["timestamp"].dt.time >= config.force_exit_start_time)
    ...
    data["late_sell_signal"] = (
        ...
        & (data["slope_sell"] > 0)
        & data["slope_slowing_enough"]
        & (data["timestamp"].dt.time >= config.late_sell_start_time)
    )
...
def create_trade_log(data, start_date=None, end_date=None):
    ...
    for trading_date, day_data in data.groupby("date"):
            ...
            trade_rows.append(
                {
                    ...
                    "setup_time": (
                        reference_row["timestamp"] if not setup_rows.empty else pd.NaT
                    ),
                    "buy_time": pd.NaT,
                    "buy_price": np.nan,
                    "sell_time": pd.NaT,
        ...
        if sell_rows.empty:
            ...
            exit_type = "force_exit" if sell_row["force_exit"] else "profit_sell"
...
def run_strategy_on_data(raw_data, config):
    ...
    featured_data = add_gap_features(featured_data)

    regular_data = featured_data[featured_data["session"] == "regular"].copy()
...
def plot_all_trade_days(data, config):
    if config.clear_existing_graphs:
        clear_graph_files(config.output_folder, config)
...
def copy_trade_days_to_folder(dates, output_folder, config):
    output_folder.mkdir(parents=True, exist_ok=True)
    ...
    for trading_date in dates:
        ...
        source_path = config.output_folder / file_name
...
def non_executed_setup_dates(data):
    ...
    buy_price_days = data.groupby("date")["buy_price"].apply(lambda x: x.notna().any())
    sell_days = data.groupby("date")["sell_signal"].any()
    non_executed_days = setup_days & (~buy_price_days | ~sell_days)
...
def main():
    ...
    regular_data = run_strategy(config)
    ...
    if config.update_graphs:
        ...
        if config.plot_filtered_folders:
            copy_trade_days_to_folder(
                dates=negative_return_dates(regular_data),
                output_folder=config.negative_return_folder,
                config=config,
            )

            copy_trade_days_to_folder(
                dates=non_executed_setup_dates(regular_data),
